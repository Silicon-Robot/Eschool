...
class Questionnaire extends Component {
    ...
    handleSaveEvaluation=()=>{
        ...
        */
        console.log(evaluationToCreate)
         fetch(`https://dp-db.herokuapp.comteacher/questionnaire/new`, {
                         method: 'post',
    ...
    handleDeleteClick=(e)=>{
        ...
        fetch(`https://dp-db.herokuapp.comteacher/questionnaire/${idDeleteObject}/delete`, {
            method: 'delete',
            headers: {'Content-Type': 'application/json','x-access-token':window.localStorage.getItem("token")}
          })
          .then(response=>response.json())
    ...
    handleSaveClick=(evalx)=>{
        ...
        */
        ...
        delete evaluation.idEvaluation
         fetch(`https://dp-db.herokuapp.comteacher/questionnaire/${evalx.idEvaluation}/update`, {
                         method: 'put',
                         headers: {'Content-Type': 'application/json','x-access-token':window.localStorage.getItem("token")},
                         body: JSON.stringify({
    ...
    fileChanged= async (e)=> {
        ...
        let data = await fetch('https://dp-db.herokuapp.comteacher/questionnaire/upload', {
                         method: 'post',
                ...
                return <div className="question" key={++index}>
                    ...
                    <div>
                        ...
                        {this.state.editableEvaluation.questions.length?
                            this.state.editableEvaluation.questions[index-1].refFiles.map((id,i)=><img key={i} src={"https://dp-db.herokuapp.comteacher/questionnaire/image/"+id} style={{height: "100px",width: "100px"}} alt="uploaded"/>)
                        :null}
                    </div>
    ...
    componentDidMount(){
        fetch('https://dp-db.herokuapp.comteacher/questionnaire/evaluation-classes-courses', {
            method: 'get',
            headers: {'Content-Type': 'application/json','x-access-token':window.localStorage.getItem("token")}
          })
          .then(response=>response.json())
